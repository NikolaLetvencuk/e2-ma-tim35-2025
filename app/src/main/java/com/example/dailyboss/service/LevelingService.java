...
public class LevelingService {
     ...
     * Level 1: 0 PP (početni nivo)
     * Level 2: 40 PP (prva nagrada)
     * Level 3+: current PP * 2.75
     * 
     * Primeri:
     * Level 1: 0 PP
     * Level 2: 40 PP
     * Level 3: 110 PP (40 * 2.75)
     * Level 4: 303 PP (110 * 2.75)
     */
    public static int calculatePPRewardForLevel(int level) {
        if (level <= 1) return 0;  // Level 1 je početni, nema PP nagradu
        if (level == 2) return FIRST_LEVEL_PP_REWARD;  // Level 2 dobija prvu nagradu (40 PP)

        // Za Level 3+: PP prethodnog nivoa * 2.75
        int previousLevelPP = calculatePPRewardForLevel(level - 1);
        return (int) Math.round(previousLevelPP * 2.75);
    }
    ...
    public boolean addExperiencePoints(String userId, int xpToAdd) {
        ...
        if (leveledUp) {
            ...
            // Postavi PP na ukupnu nagradu za novi nivo
            int newLevelPP = calculatePPRewardForLevel(currentLevel);
            stats.setPowerPoints(newLevelPP);
            
            // Ažuriraj titulu
            stats.setTitle(getTitleForLevel(currentLevel));

            Log.d(TAG, String.format("User leveled up! %d -> %d. New PP: %d", 
                    oldLevel, currentLevel, newLevelPP));
        }
    ...
    /**
     * Resetuje PP korisnika na ukupnu nagradu za trenutni nivo (koristi se nakon borbe).
     * 
     * Primeri:
     * Level 1: 0 PP
     * Level 2: 40 PP
    ...
    public boolean resetPowerPoints(String userId) {
        ...
        int totalPPReward = calculatePPRewardForLevel(userProfile.getLevel());
        
        Log.d(TAG, String.format("Resetting PP for level %d: total = %d", userProfile.getLevel(), totalPPReward));
    ...
    /**
     * Test metoda za proveru PP formule
     */
    public static void testPPFormula() {
        Log.d(TAG, "=== PP Formula Test ===");
        for (int level = 1; level <= 10; level++) {
            int ppReward = calculatePPRewardForLevel(level);
            Log.d(TAG, String.format("Level %d: %d PP", level, ppReward));
        }
        Log.d(TAG, "=== End PP Formula Test ===");
    }
